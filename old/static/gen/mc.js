(() => {
  ...
  canvas.addEventListener("mousemove", (e2) => {
    if (isDragging) {
      offsetX += e2.offsetX - lastX;
      offsetY += e2.offsetY - lastY;
      lastX = e2.offsetX;
      lastY = e2.offsetY;
      draw();
    }
  ...
  canvas.addEventListener("wheel", (e2) => {
    e2.preventDefault();
    const delta = e2.deltaY < 0 ? 1.1 : 0.9;
  ...
  canvas.addEventListener("touchstart", (e2) => {
    if (e2.touches.length === 1) {
      isDragging = true;
      lastX = e2.touches[0].clientX;
      lastY = e2.touches[0].clientY;
    }
  });
  canvas.addEventListener("touchmove", (e2) => {
    if (e2.touches.length === 1 && isDragging) {
      ...
      if (window.lastDist) {
        const scaleDelta = distance / window.lastDist;
        scale *= scaleDelta;
        const rect = canvas.getBoundingClientRect();
        const mx = (e2.touches[0].clientX + e2.touches[1].clientX) / 2 - rect.left;
        const my = (e2.touches[0].clientY + e2.touches[1].clientY) / 2 - rect.top;
        offsetX = mx - (mx - offsetX) * scaleDelta;
        offsetY = my - (my - offsetY) * scaleDelta;
        draw();
      }
      window.lastDist = distance;
    }
  });
  ...
  setTimeout(updateImage, 1e3 * 60 * 4);
})();
